function [] = sbc1_Analysis(sigsToMeasure, rawSigsToMeasure,  tvals, cellIDs, approxStart, approxEnd, viewPlots, saveFlag, saveDir)

[~, approxStartInd] = min(abs(tvals-approxStart));
[~, approxEndInd] = min(abs(tvals-approxEnd));

tRiseSBC1 = zeros(1,size(sigsToMeasure,1));
...
for pp = 1:size(sigsToMeasure,1)
    ...
    if ~isempty(dfPk)
        ...
        tRiseSBC1(pp) = (sigEnd(pp)-sigStart(pp))/2;
...
%         noiseSTD = std(abs(rawSignal-max(rawSignal(:)).*tmpData));
        ...
        tRiseSBC1(pp) = halfTime - approxStart;
...
tsRiseSBC1Cleaned(riseIntDiff<=0) = [];
sigEndCleaned(riseIntDiff<=0) =[];
cellIDsCleaned(riseIntDiff<=0) = [];
ampsCleaned(riseIntDiff<=0) = [];
SBC1CleanedRaw(riseIntDiff<=0,:) = [];

tsRiseSBC1Cleaned(tsRiseSBC1Cleaned<=0) = [];
sigEndCleaned(tsRiseSBC1Cleaned<=0) = [];
cellIDsCleaned(tsRiseSBC1Cleaned<=0) = [];
ampsCleaned(tsRiseSBC1Cleaned<=0) = [];
SBC1CleanedRaw(tsRiseSBC1Cleaned<=0,:) = [];

cellIDsCleaned = ['Time' cellIDsCleaned]; 

sigStats = [mean(tsRiseSBC1Cleaned) std(tsRiseSBC1Cleaned); mean(sigEndCleaned-approxStart) std(sigEndCleaned-approxStart); ...
    mean(ampsCleaned) std(ampsCleaned)];

% Display Histograms of Rise Half-Time
figure; histogram(tsRiseSBC1Cleaned); 
    xlabel('Time (sec)'); 
    ylabel('Frequency'); 
    title('\tau_{1/2} Decay Times - SBC1');
...
fprintf(fid,'Cell Type: SBC1\n');
fprintf(fid,'N = %d\n', length(tsRiseSBC1Cleaned));
...
T = num2cell([tsRiseSBC1Cleaned; ampsCleaned]);
